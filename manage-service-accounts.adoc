...
---
= NetApp Workload Factory에서 서비스 계정 관리
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
. 선택적으로 클라이언트 자격 증명 교환을 실행하여 Auth0 관리 API용 액세스 토큰을 얻을 수 있습니다. 새로 고침 토큰 생성에 대한 자세한 내용은 link:https://docs.netapp.com/us-en/console-automation/platform/grant_types.html#refresh-token["NetApp Console API 설명서"^]을 참조하십시오.
+
이 curl 예제는 클라이언트 ID와 시크릿 키를 사용하여 API를 통해 시간 제한이 있는 액세스 토큰을 생성하는 방법을 보여줍니다. 이 토큰은 NetApp Workload Factory API에 몇 시간 동안 액세스할 수 있는 권한을 제공합니다.
...
----
+
출력 결과를 https://netapp-cloud-account.auth0.com/oauth/token에 게시하여 액세스 토큰을 받으세요.
