...
[role="quick-margin-para"]
이 단계는 선택 사항입니다. AWS 계정 자격 증명을 추가하지 않고도 Workload Factory를 사용할 수 있습니다. Workload Factory에 AWS 자격 증명을 추가하면 Workload Factory 계정에 FSx for ONTAP 파일 시스템을 생성 및 관리하고 데이터베이스와 같은 특정 워크로드를 배포 및 관리하는 데 필요한 권한이 부여됩니다.
